...
const Player = (name, value) => {
  let play = () => {
    ...
    if (!gameFlow.determineWinner(gameboard.gameboardValues)) {
      ...
      let availableSquare = gameSquare.filter(
        (square) => square.textContent === ""
      );
      availableSquare.forEach((square) =>
        square.addEventListener("click", (e) => {
          if (
            !triggered &&
            !gameFlow.determineWinner(gameboard.gameboardValues)
          ) {
            /* triggered = true; */
...
const gameFlow = (() => {
  ...
  const determineWinner = (gameArray) => {
    ...
    const winningPlayerAlert = document.querySelector(".current-player");
